	...
	for (j = 0; j < ncol; j++) {
		for (i = 0; i < nrow; i++) {
			if (img[i + j * nrow]) {
				...
				minjunc = -1;
				for (ijunc = 0; ijunc < njunc; ijunc++) {
					int ijy = SQR(i - jy[ijunc]);
					int jjx = SQR(j - jx[ijunc]);
					int j1jx = SQR(j + 1 - jx[ijunc]);
					int i1jy = SQR(i + 1 - jy[ijunc]);
				...
				nnear = pspan = 0;
				int min1 = MIN(mindNE, dNE[minjunc]);
				int min2 = MIN(mindNW, dNW[minjunc]);
				int min3 = MIN(mindSE, dSE[minjunc]);
				int min4 = MIN(mindSW, dSW[minjunc]);
				int edgej_minjunc = edgej[minjunc];
				for (ijunc = 0; ijunc < njunc; ijunc++) {
					if ((dNE[ijunc] <= min1)
						|| (dNW[ijunc] <= min2)
						|| (dSE[ijunc] <= min3)
